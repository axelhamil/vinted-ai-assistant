...
### 2026-01-18 - Task 24: Implémenter Export Markdown

**Status:** Completed

**Files Modified:**
- `apps/extension/src/background/index.ts` - Enhanced exportMarkdown function to extract filename from Content-Disposition header
- `apps/extension/src/content/App.tsx` - Updated handleExport to use filename from backend response
...
**Export Markdown Features:**

**Button in Sidebar:**
- Export .md button already existed in sidebar footer (from Task 21)
- Shows loading spinner during export
- Triggers file download on success

**Backend Integration:**
- Calls backend endpoint `/api/analyses/:vintedId/export`
- Backend returns markdown content with Content-Disposition header containing filename
- Filename follows PRD format: `{brand}_{title}_{date}.md`

**File Download:**
- Creates Blob from markdown content
- Generates object URL for download
- Triggers download with proper filename from backend
- Cleans up object URL after download

**Technical Notes:**
- Added `ExportMarkdownResponse` interface with content and filename properties
- Background worker now parses Content-Disposition header to extract filename
- Filename is generated by backend's ExportMarkdownUseCase (from Task 13)
- Fallback filename "analysis.md" if header parsing fails

### 2026-01-18 - Task 25: Implémenter cache IndexedDB (Dexie)

**Status:** Completed

**Files Created:**
- `apps/extension/src/db/index.ts` - Dexie database with cache operations

**Files Modified:**
- `apps/extension/src/content/App.tsx` - Integrated local cache with TTL checks
- `apps/extension/src/content/components/Sidebar.tsx` - Added cache info display and refresh loading state
...
**Dexie Database (`src/db/index.ts`):**
- `AnalysisCacheDb` class extending Dexie with `analyses` table
- `CachedAnalysis` interface with vintedId (primary key), analysis data, cachedAt, expiresAt
- Primary index on `vintedId`, secondary index on `expiresAt` for cleanup queries

**Cache Operations:**
- `getCachedAnalysis(vintedId)` - Get cached analysis, returns null if expired
- `cacheAnalysis(analysis)` - Cache analysis with TTL (1 hour)
- `invalidateCache(vintedId)` - Delete specific cache entry
- `clearExpiredCache()` - Delete all expired entries
- `clearAllCache()` - Clear entire cache
- `getCacheStats()` - Get total, expired, and valid entry counts
- `isCached(vintedId)` - Check if analysis is cached and valid
- `getCacheTimeRemaining(vintedId)` - Get milliseconds until expiration

**Cache Integration in App.tsx:**
- Added `cacheInfo` state tracking cache status and time remaining
- `checkCachedAnalysis()` now checks local IndexedDB first, then backend
- Automatically caches backend responses locally
- `analyzeArticle()` now caches new analysis results
- `handleRefresh()` invalidates cache before re-analysis (force refresh)

**UI Updates in Sidebar.tsx:**
- Added `cacheInfo` and `isRefreshing` props
- `formatCacheTime()` helper for human-readable time display (min/heures)
- Footer shows "Depuis le cache (expire dans X min)" when cached
- Green indicator dot when serving from cache
- Refresh button shows loading spinner during re-analysis
- Refresh button disabled while refreshing

**Technical Notes:**
- Cache TTL is 1 hour (CACHE_TTL_MS = 60 * 60 * 1000)
- Database name: 'vinted-ai-cache', version 1
