...
export function NegotiateTab({ analysis }: NegotiateTabProps) {
	...
	const belowMarket = Math.round(
		((marketPrice.average - negotiation.suggestedOffer) / marketPrice.average) * 100
	)
		...
		<div className="space-y-4 animate-fade-in">
			...
			>
				<div className="space-y-3">
					<CardRow label="Prix demandé" value={`${price}€`} />
					<CardRow
						label="Votre offre"
					...
					<div className="pt-3 border-t border-border">
						<CardRow label="Prix moyen marché" value={`${marketPrice.average}€`} />
						<div className="mt-2 flex items-center gap-2">
							...
							<span className="text-lg text-profit">{belowMarket}% sous le prix marché</span>
						</div>
					</div>
				</div>
			</Card>
