 ...
 */
function getLightingLabel(lighting: PhotoQuality['lighting']): {
	label: string
 ...
 */
function getBackgroundLabel(background: PhotoQuality['background']): {
	label: string
	variant: 'success' | 'warning' | 'negative'
} {
	switch (background) {
			...
			<div className="flex items-start gap-4">
				...
				<div className="flex-1 space-y-3">
					...
					<div className="flex flex-wrap gap-2">
						<Pill variant={lightingInfo.variant}>{lightingInfo.label}</Pill>
						<Pill variant={backgroundInfo.variant}>{backgroundInfo.label}</Pill>
						{hasModel && (
					...
					{issues.length > 0 && (
						<div className="space-y-1">
							{issues.slice(0, 2).map((issue) => (
								<div
									key={issue}
									className="flex items-start gap-1.5 text-lg text-content-secondary"
								>
