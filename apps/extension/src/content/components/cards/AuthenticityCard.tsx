...
export function AuthenticityCard({ score, flags, confidence }: AuthenticityCardProps) {
	...
	const displayedFlags = isExpanded ? flags : flags.slice(0, 3)
					...
					{hasIssues ? (
						<div className="space-y-2">
							<div className="flex items-center gap-2">
								<svg
									aria-hidden="true"
									className="w-5 h-5 text-caution flex-shrink-0"
									fill="currentColor"
									viewBox="0 0 20 20"
								>
									<path
							...
							<ul className="space-y-1">
								{displayedFlags.map((flag) => (
									<li
										key={flag}
										className="text-lg text-content-secondary flex items-start gap-1.5"
									>
								...
								{hasMoreFlags && (
									<li>
										<button
											...
											className="text-base text-primary hover:text-primary-hover cursor-pointer transition-colors"
										>
											{isExpanded ? 'Voir moins' : `+${flags.length - 3} autres...`}
										</button>
									</li>
								)}
							</ul>
						...
						<div className="flex items-center gap-2">
							...
							<span className="text-xl text-profit">Aucun problème détecté</span>
						</div>
					)}
				</div>
			</div>
